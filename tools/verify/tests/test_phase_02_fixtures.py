...
EXPECTED_FIXTURE_MANIFEST_SHA256 = "0" * 64
...
def _assert_inventory(test: unittest.TestCase, archive: Path, manifest: dict[str, Any]) -> None:
    ...
    for relative_path, expected in inventory.items():
        ...
        test.assertEqual(expected, _file_binding(candidate, archive))


def _assert_journal(test: unittest.TestCase, archive: Path, manifest: dict[str, Any]) -> None:
    journal = manifest["journal"]
    test.assertEqual(list(range(len(journal))), [record["sequence"] for record in journal])
    for record in journal:
        test.assertEqual(
            _canonical_digest(record, omit="record_digest_sha256"),
            record["record_digest_sha256"],
        )
        if record["record_type"] == "event":
            payload = archive / record["payload_path"]
            test.assertEqual(record["payload_sha256"], _sha256(payload.read_bytes()))
        else:
            packet = archive / record["frame_packet_path"]
            test.assertEqual(record["frame_packet_sha256"], _sha256(packet.read_bytes()))
...
def _validate_archive(test: unittest.TestCase, archive: Path, oracle: dict[str, Any]) -> None:
    ...
    test.assertEqual(
        _canonical_digest(manifest, omit="manifest_sha256"), manifest["manifest_sha256"]
    )
    _assert_inventory(test, archive, manifest)
    _assert_journal(test, archive, manifest)

    test.assertEqual(
        [event["journal_sequence"] for event in manifest["events"]],
        sorted(event["journal_sequence"] for event in manifest["events"]),
    )
    test.assertEqual(
        [frame["journal_sequence"] for frame in manifest["accepted_frames"]],
        sorted(frame["journal_sequence"] for frame in manifest["accepted_frames"]),
    )

    for accepted in manifest["accepted_frames"]:
        packet_path = archive / accepted["frame_packet_path"]
        packet = _load_json(packet_path)
        _assert_schema(test, CON001_SCHEMA, packet)
        test.assertEqual(accepted["frame_packet_sha256"], _sha256(packet_path.read_bytes()))
        image_path = archive / packet["image"]["rrcap_file"]["relative_path"]
        test.assertEqual(packet["image"]["payload_length"], image_path.stat().st_size)
        test.assertEqual(packet["image"]["payload_sha256"], _sha256(image_path.read_bytes()))
        test.assertEqual(accepted["frame_id"], packet["frame_id"])
        test.assertEqual(accepted["timestamp_ns"], packet["timestamp_ns"])
        test.assertEqual(accepted["revision_id"], packet["scene_context"]["revision_id"])

    expected = oracle["expected"]
    test.assertEqual(expected["finalization_state"], manifest["finalization"]["state"])
    test.assertEqual(expected["accepted_frame_count"], len(manifest["accepted_frames"]))
    ...
    test.assertEqual(expected["journal_tuple_sha256"], manifest["replay"]["journal_tuple_sha256"])
    test.assertEqual(expected["frame_projection_sha256"], _projection_digest(manifest, "accepted_frames"))
...
def _validate_fixture_root(test: unittest.TestCase, root: Path) -> None:
    ...
    test.assertEqual(
        {
            "session_id": "session_consent_denied_0001",
