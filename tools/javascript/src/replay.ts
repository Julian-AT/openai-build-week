...
export const EXACT_NODE_VERSION = "v22.22.3";
...
function makeReport(
  ...
  const report: JsonObject = {
    ...
    evaluator: { name: "ReRoomReplayNode", version: "1.0.0", platform: "javascript" },
    fixture: {
...
async function publishReports(reports: Map<string, Buffer>, outputRoot: string): Promise<void> {
  ...
  const stage = path.join(parent, `.reroom-node-replay-staging-${randomUUID()}`);
  let published = false;
  try {
...
export async function runReplay(options: ReplayOptions): Promise<void> {
  requireReplay((options.runtimeVersion ?? process.version) === EXACT_NODE_VERSION, "exact Node v22.22.3 is required before replay");
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    ...
    process.stderr.write(`replay-node: FAIL: ${message}\n`);
    process.exitCode = 1;
  }
}
