...
import { EXACT_NODE_VERSION, runReplay } from "../src/replay.ts";
...
async function temporaryDirectory() {
  return mkdtemp(path.join(os.tmpdir(), "reroom-node-replay-test-"));
}
...
test("exact Node independently emits the complete closed replay corpus", async () => {
  ...
  for (const [name, bytes] of reports) {
    ...
    assert.deepEqual(report.evaluator, { name: "ReRoomReplayNode", platform: "javascript", version: "1.0.0" });
...
test("sequential and concurrent isolated Node runs are byte-identical", async () => {
  const temporary = await temporaryDirectory();
