...
function assertComplete(result, bytes) {
  ...
  assert.deepEqual(result.runtime, {
    language: "typescript",
    name: "ReRoomTransactionNode",
    version: "node-v22.22.3",
  });
...
test("exact Node independently emits the complete provenance-bound transaction corpus", async () => {
  assert.equal(process.version, EXACT_NODE_VERSION);
...
test("two isolated Node publications are byte-identical", async () => {
...
test("runtime revision fixture and oracle drift reject before publication", async () => {
  await assert.rejects(
    produceTransactionTrace({ manifestPath: MANIFEST, repoRoot: ROOT, implementationRevision: REVISION, runtimeVersion: "v22.22.2" }),
    /Node v22\.22\.3 is required/,
  );
  await assert.rejects(
...
test("producer source is closed and never imports another runtime output", async () => {
  const source = await readFile(path.join(ROOT, "tools/javascript/src/transaction.ts"), "utf8");
