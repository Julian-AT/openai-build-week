...
import { runReplay } from "../../tools/javascript/src/replay.ts";
...
function loaderProbeSource(scenario) {
  const loaderUrl = pathToFileURL(path.join(WEB_ROOT, "src/lib/replay/load-golden-capture.server.ts")).href;
  return `
    import { execFile as execFileCallback } from "node:child_process";
    import { access, mkdir, mkdtemp, readFile, rm, writeFile } from "node:fs/promises";
    import { promisify } from "node:util";
    const execFile = promisify(execFileCallback);
    const { loadGoldenCapture, __loadGoldenCaptureForTesting } = await import(${JSON.stringify(loaderUrl)});
    ...
    const result = await __loadGoldenCaptureForTesting({
      ...
      execFile: async (executable, args, options) => {
        ...
        if (scenario === "malformed_report") {
          ...
          await writeFile(new URL(${JSON.stringify(REPORT_NAME)}, "file://" + outputRoot + "/"), "{not json");
    ...
    if (temporaryParent !== null) {
      try { await access(temporaryParent); temporaryParentExists = true; } catch {}
    }
    process.stdout.write(JSON.stringify({ result, invocation, temporaryParentExists, publicArity: loadGoldenCapture.length }));
  `;
}

function runLoaderProbe(scenario) {
  const resolveHook = `
  ...
  `;
  const hookUrl = `data:text/javascript,${encodeURIComponent(resolveHook)}`;
  const completed = spawnSync(
    process.execPath,
    ["--experimental-loader", hookUrl, "--input-type=module", "--eval", loaderProbeSource(scenario)],
...
test("server-only loader invokes the exact fixed CLI and cleans temporary output", () => {
  const probe = runLoaderProbe("success");
  ...
  assert.deepEqual(probe.invocation.args, [
    path.join(REPO_ROOT, "tools/javascript/src/replay.ts"),
    "--manifest",
...
test("runner, report, JSON, and cleanup failures return one sanitized rejection", () => {
  for (const scenario of ["runner_failure", "missing_report", "malformed_report", "cleanup_failure"]) {
    const probe = runLoaderProbe(scenario);
