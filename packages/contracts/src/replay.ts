...
const PINNED_FIXTURE_MANIFEST_SHA256 =
  "3b4519d2730e158df73e938f7b841664c6ce5f7d65ed2650c90ca8e89c7a7610";
const PINNED_REPORT_SCHEMA_SHA256 =
...
function subsetKeys(
  ...
  requireReplay(
    [...required].every((key) => actual.has(key)) && [...actual].every((key) => allowed.has(key)),
    message,
  );
}
...
async function safeExisting(
  ...
  requireReplay(
    ...
    `archive ${expected} has the wrong type`,
  );
  return candidate;
}
...
async function verifyFixtureInventory(fixture: JsonObject, fixtureRoot: string): Promise<void> {
  ...
  for (const rawEntry of inventory) {
    ...
    exactKeys(
      entry,
      new Set(["byte_length", "relative_path", "sha256"]),
      "fixture file binding is not closed",
    );
    const relativePath = entry.relative_path;
    requireReplay(
      typeof relativePath === "string" && DIGEST.test(entry.sha256),
      "fixture file identity is invalid",
    );
    paths.push(relativePath);
    const bytes = await regularBytes(
      await safeExisting(fixtureRoot, relativePath, "file"),
      exactInteger(entry.byte_length) + 1,
    );
    requireReplay(
      bytes.length === entry.byte_length && sha256Hex(bytes) === entry.sha256,
      "fixture file digest mismatch",
    );
  }
  lexicalUnique(paths, "fixture file inventory is duplicated or unsorted");
  const physical = (
    await allRegularFiles(await safeExisting(fixtureRoot, "archives", "directory"))
  ).map((candidate) => path.relative(fixtureRoot, candidate).split(path.sep).join("/"));
  requireReplay(
    JSON.stringify(physical) === JSON.stringify(paths),
    "fixture raw inventory is incomplete or contains an extra file",
  );
  ...
  for (const rawEntry of directories) {
    ...
    exactKeys(
      entry,
      new Set(["byte_length", "file_count", "relative_path", "tree_sha256"]),
      "fixture directory binding is not closed",
    );
    const relativePath = entry.relative_path;
    requireReplay(
      typeof relativePath === "string" && DIGEST.test(entry.tree_sha256),
    ...
    const bindings = await Promise.all(
      (await allRegularFiles(directory)).map((candidate) => fileBinding(candidate, fixtureRoot)),
    );
    requireReplay(bindings.length === entry.file_count, "fixture directory file count mismatch");
    requireReplay(
      bindings.reduce((total, binding) => total + binding.byte_length, 0) === entry.byte_length,
      "fixture directory byte length mismatch",
    );
    requireReplay(
      canonicalDigest(bindings) === entry.tree_sha256,
...
async function loadFixture(
...
): Promise<{ fixture: JsonObject; fixtureRoot: string }> {
  ...
  requireReplay(
    sha256Hex(manifestBytes) === PINNED_FIXTURE_MANIFEST_SHA256,
    "capture fixture manifest drifted",
  );
  const fixture = object(
    parseJsonBytesStrict(manifestBytes),
    "capture fixture manifest is not an object",
  );
  ...
  requireReplay(
    reportSchema.sha256 === PINNED_REPORT_SCHEMA_SHA256,
    "report schema binding drifted",
  );
  const schemaPath = await safeExisting(
    path.resolve(options.repoRoot),
    reportSchema.relative_path,
    "file",
  );
  const schemaBytes = await regularBytes(schemaPath, exactInteger(reportSchema.byte_length) + 1);
  requireReplay(
    schemaBytes.length === reportSchema.byte_length &&
      sha256Hex(schemaBytes) === PINNED_REPORT_SCHEMA_SHA256,
    "report schema drifted",
  );
  const schema = object(parseJsonBytesStrict(schemaBytes), "report schema is invalid JSON");
  requireReplay(
    schema.$id === "https://reroom.dev/schemas/replay-report/1.0.0" &&
  ...
  requireReplay(
    JSON.stringify(archiveNames) === JSON.stringify(ARCHIVE_NAMES),
  ...
  requireReplay(
    JSON.stringify(probeIDs) === JSON.stringify(PROBE_IDS),
  ...
  requireReplay(
    consent.archive_created === false &&
      consent.consent_granted === false &&
      consent.expected_verdict === "reject" &&
      consent.rejection_class === "semantic_invariant" &&
      typeof consent.session_id === "string" &&
      SESSION_ID.test(consent.session_id),
...
async function replayArchive(descriptor: JsonObject, fixtureRoot: string): Promise<ReplaySnapshot> {
  ...
  const manifestBytes = await regularBytes(
    await safeExisting(archiveRoot, "manifest.json", "file"),
  );
  const manifest = object(parseJsonBytesStrict(manifestBytes), "archive manifest is invalid JSON");
  requireReplay(
    canonicalizeBytes(manifest).equals(manifestBytes),
  ...
  requireReplay(
    manifest.format_version === "1.0.0" && manifest.capture_kind === "native_arkit",
    "unsupported archive contract version",
  );
  requireReplay(
    typeof manifest.session_id === "string" && SESSION_ID.test(manifest.session_id),
  ...
  requireReplay(
    finalization.manifest_sha256_algorithm === "RR-JCS-SHA256-1" &&
      finalization.manifest_sha256_scope ===
  ...
  requireReplay(
    canonicalDigest(unsignedManifest) === finalization.manifest_sha256,
  ...
  requireReplay(
    journal.length > 0 && journal.length <= MAX_CASES,
    "archive journal count is invalid",
  );
  ...
  for (const [index, rawEvent] of events.entries()) {
    ...
    requireReplay(
      exactInteger(event.event_sequence) === index &&
        durable < journal.length &&
        typeof event.event_id === "string" &&
        typeof event.payload_path === "string" &&
        DIGEST.test(event.payload_sha256) &&
        DIGEST.test(event.record_sha256) &&
  ...
  for (const [index, rawFrame] of frames.entries()) {
    ...
    requireReplay(
      exactInteger(frame.sequence) === index &&
        durable < journal.length &&
        typeof frame.frame_id === "string" &&
        typeof frame.packet_path === "string" &&
        DIGEST.test(frame.packet_sha256),
      "frame projection member is invalid",
    );
    const packetBinding = inventory.get(frame.packet_path);
    requireReplay(
      packetBinding?.sha256 === frame.packet_sha256,
      "frame packet is not bound by inventory",
    );
    const journalEntry = object(journal[durable]);
    requireReplay(
      journalEntry.entry_type === "frame" &&
        journalEntry.reference_id === frame.frame_id &&
    ...
    const packetBytes = await regularBytes(
      await safeExisting(archiveRoot, frame.packet_path, "file"),
    );
    const packet = object(parseJsonBytesStrict(packetBytes), "frame packet is invalid JSON");
    requireReplay(
      canonicalizeBytes(packet).equals(packetBytes),
      "frame packet is not exact JCS bytes",
    );
    ...
    requireReplay(
      packet.protocol_version === "1.0.0" &&
        packet.coordinate_convention === "RR-COORD-1" &&
        packet.frame_id === frame.frame_id &&
        exactInteger(packet.capture_sequence) === index &&
        image.codec === "png" &&
        payload.kind === "rrcap_file" &&
        typeof payload.relative_path === "string" &&
        DIGEST.test(payload.sha256) &&
        packet.payload_sha256 === payload.sha256,
      "frame packet identity or payload binding is invalid",
    );
    const imageBinding = inventory.get(payload.relative_path);
    requireReplay(
      imageBinding?.sha256 === payload.sha256 &&
        imageBinding?.byte_length === exactInteger(payload.byte_length),
  ...
  for (const rawFrame of frames) {
    ...
    const packet = object(
      parseJsonBytesStrict(
        await regularBytes(await safeExisting(archiveRoot, frame.packet_path, "file")),
      ),
    );
  ...
  const journalTupleSHA256 = canonicalDigest(
    journal.map((raw) => {
  ...
  requireReplay(
    replay.ordering_authority === "global_journal_sequence" &&
      ...
      replay.input_digest_scope ===
        "jcs_array_of_journal_sequence_entry_type_reference_id_content_sha256" &&
      array(replay.provider_lock).length === 0 &&
      replay.input_digest === journalTupleSHA256,
    "journal tuple input digest mismatch",
  );
  requireReplay(
    exactInteger(finalization.last_durable_journal_sequence) === journal.length - 1,
  ...
  requireReplay(
    expected.verdict === "accept" && expected.rejection_class === null,
...
function contiguous(values: unknown): boolean {
  return (
    ...
    values.every((value, index) => Number.isSafeInteger(value) && value === index)
  );
}
...
function evaluateProbe(probe: JsonObject): CaseOutcome {
  ...
  switch (probe.case_id) {
    case "fr-b0.adjacency":
      outcome =
        Number.isSafeInteger(input.prior_revision) &&
        input.next_revision === input.prior_revision + 1
    ...
    case "fr-b0.concurrency": {
      ...
      outcome =
        readerCount >= 2 && contiguous(sequences) && new Set(digests).size === 1
          ? { verdict: "accept", rejectionClass: null }
          : { verdict: "reject", rejectionClass: "semantic_invariant" };
      break;
    }
    case "fr-b0.empty":
      outcome =
        input.accepted_frame_count === 0 && exactInteger(input.event_count) > 0
          ? { verdict: "accept", rejectionClass: null }
          : { verdict: "reject", rejectionClass: "semantic_invariant" };
      break;
    case "fr-b0.ordering":
    ...
    case "fr-capture.adjacency": {
      ...
      outcome =
        input.frame_ids_distinct === true && contiguous(sequences)
          ? { verdict: "accept", rejectionClass: null }
          : { verdict: "reject", rejectionClass: "semantic_invariant" };
      break;
    }
    case "fr-capture.boundary": {
      ...
      const neighborVerdicts = array(input.neighbors).map((value) =>
        exactInteger(value) <= maximum ? "accept" : "reject",
      );
    ...
    case "fr-capture.empty":
      outcome =
        exactInteger(input.image_byte_length) > 0
          ? { verdict: "accept", rejectionClass: null }
    ...
    case "fr-capture.ordering": {
      ...
      const valid =
        positions.every(
          (value, index) => value >= 0 && (index === 0 || positions[index - 1] < value),
        ) &&
    ...
    case "fr-capture.precision": {
      requireReplay(
        typeof input.binary32_hex === "string" && /^[0-9a-f]{8}$/.test(input.binary32_hex),
        "precision probe binary32 bytes are invalid",
      );
      const bytes = Buffer.from(input.binary32_hex, "hex");
      const value = bytes.readFloatBE(0);
      outcome =
        Number.isFinite(value) && Object.is(value, Math.fround(input.decimal_input))
          ? { verdict: "accept", rejectionClass: null }
    ...
    case "nfr-replay.assumption":
      outcome =
        exactInteger(input.controlled_capture_minutes) > 0 &&
        input.evidence_classification === "HYPOTHESIS"
          ? { verdict: "accept", rejectionClass: null }
          : { verdict: "reject", rejectionClass: "semantic_invariant" };
      break;
    case "sec-consent.concurrent-session-separation":
      outcome =
        typeof input.candidate_session_id === "string" &&
        typeof input.consented_session_id === "string" &&
...
function makeReport(
  ...
  const report: JsonObject = {
    ...
    metrics: {
      ...
      recovered_prefix_records:
        snapshot.finalizationState === "recovered_prefix" ? snapshot.journalRecordCount : 0,
...
async function validateOutput(outputRoot: string): Promise<void> {
  try {
    ...
    if ((error as NodeJS.ErrnoException).code !== "ENOENT")
      throw new ReplayFailure("output root cannot be inspected");
  }
...
async function publishReports(reports: Map<string, Buffer>, outputRoot: string): Promise<void> {
  ...
  try {
    ...
    for (const caseID of [...reports.keys()].sort()) {
      ...
      requireReplay(report !== undefined, "replay report map is incomplete");
      await durableWrite(path.join(stage, `${caseID}.replay-report.json`), report);
    }
    const names = (await readdir(stage)).sort();
    const expected = [...reports.keys()].sort().map((caseID) => `${caseID}.replay-report.json`);
    requireReplay(
      JSON.stringify(names) === JSON.stringify(expected),
      "staged replay report set is incomplete",
    );
...
export async function runReplay(options: ReplayOptions): Promise<void> {
  requireReplay(
    ...
    "exact Bun 1.3.11 is required before replay",
  );
  requireReplay(
  ...
  for (const rawProbe of array(fixture.edge_probes)) {
    ...
    reports.set(
      probe.case_id,
      makeReport(
        probe.case_id === "fr-b0.empty" ? empty : ordinary,
        probe.case_id,
        outcome,
        options.implementationRevision,
      ),
  ...
  reports.set(
    "sec-consent.denied",
    makeReport(empty, "sec-consent.denied", consentOutcome, options.implementationRevision),
  );

  const caseIDs = [...reports.keys()].sort();
  const expectedCaseIDs = [
    ...ARCHIVE_NAMES.map((name) => `archive.${name.slice(0, -".rrcap".length)}`),
    ...PROBE_IDS,
    "sec-consent.denied",
  ].sort();
  requireReplay(
    caseIDs.length === 16 && JSON.stringify(caseIDs) === JSON.stringify(expectedCaseIDs),
...
function parseCLI(arguments_: string[]): ReplayOptions {
  ...
  requireReplay(
    ...
    "all exact replay arguments are required",
  );
  return {
    manifestPath,
    outputRoot,
