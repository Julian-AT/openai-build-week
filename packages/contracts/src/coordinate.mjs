...
export function quantizeRrFloat(value) {
  requireCoordinate(
    typeof value === "number" && Number.isFinite(value) && Math.abs(value) <= RR_FLOAT_MAX,
    "value is outside finite binary32 range",
...
export function rrFloatEqual(left, right) {
  ...
  return (
    Math.abs(a - b) <=
    SCALAR_ABSOLUTE_TOLERANCE + SCALAR_RELATIVE_TOLERANCE * Math.max(Math.abs(a), Math.abs(b))
  );
}

export function rrTranslationEqual(left, right) {
  return (
    rrFloatEqual(left, right) && Math.abs(quantizeRrFloat(left) - quantizeRrFloat(right)) <= 1e-4
  );
}

export function rrTransformedIntrinsicsEqual(left, right) {
  return (
    rrFloatEqual(left, right) && Math.abs(quantizeRrFloat(left) - quantizeRrFloat(right)) <= 1e-3
  );
}
...
function quantizedArray(values, length) {
  requireCoordinate(
    ...
    `expected ${length} matrix/vector values`,
  );
...
function validateProperRotation(rotation) {
  ...
  requireCoordinate(
    Math.sqrt(orthogonalitySquared) <= RIGID_TOLERANCE,
    "rotation is not orthonormal",
  );
  requireCoordinate(
    Math.abs(determinant3(rotation) - 1) <= RIGID_TOLERANCE,
...
export function validateRigidTransform(values) {
  ...
  requireCoordinate(
    matrix
      .slice(12)
      .every(
        (value, index) => Math.abs(value - expectedLastRow[index]) <= HOMOGENEOUS_ROW_TOLERANCE,
      ),
    "homogeneous last row is invalid",
  );
  return matrix;
}
...
function multiplyMatrixVector(matrix, vector, size) {
  ...
  for (let row = 0; row < size; row += 1) {
    for (let column = 0; column < size; column += 1)
      output[row] += matrix[row * size + column] * vector[column];
  }
...
export function projectCameraPoint(intrinsics, cameraPoint) {
  ...
  return {
    encoded_pixel: [fx * (x / z) + cx, fy * (y / z) + cy].map((value) =>
...
export function transformIntrinsics(sensorIntrinsics, encodedFromSensor, encodedSize, orientation) {
  ...
  requireCoordinate(
    Math.abs(transform[6]) <= HOMOGENEOUS_ROW_TOLERANCE &&
      Math.abs(transform[7]) <= HOMOGENEOUS_ROW_TOLERANCE &&
      Math.abs(transform[8] - 1) <= HOMOGENEOUS_ROW_TOLERANCE,
    "encoded pixel transform is not affine",
  );
  ...
  const matrix = [
    fx,
    0,
    quantizeRrFloat(sensorIntrinsics.cx),
    0,
    fy,
    quantizeRrFloat(sensorIntrinsics.cy),
    0,
    0,
    1,
  ];
  ...
  for (let row = 0; row < 3; row += 1) {
    for (let column = 0; column < 3; column += 1) {
      for (let index = 0; index < 3; index += 1)
        product[row * 3 + column] += transform[row * 3 + index] * matrix[index * 3 + column];
    }
  }
...
export function applyWorldCorrection(fromVersion, toVersion, transform, pointFrom) {
  requireCoordinate(
    ...
    "world correction must target a strictly newer epoch",
  );
...
export function executeCoordinateOperation(input) {
  switch (input.operation) {
    case "project":
      requireCoordinate(
        input.pixel_center === "half_integer",
        "pixel-center convention is unsupported",
      );
      return projectCameraPoint(input.intrinsics, input.camera_point);
    case "transform_intrinsics":
      return transformIntrinsics(
        input.sensor_intrinsics,
        input.encoded_from_sensor,
        input.encoded_size,
        input.orientation,
      );
    case "arkit_to_opencv_camera":
      return {
        opencv_camera_point: multiplyMatrixVector(
          validateProperRotation(quantizedArray(input.conversion, 9)),
          quantizedArray(input.camera_point, 3),
          3,
        ),
      };
    case "apply_world_correction":
      return {
        point_to: applyWorldCorrection(
          input.from_world_frame_version,
          input.to_world_frame_version,
          input.to_from_from_transform,
          input.point_from,
        ),
      };
...
export async function executeCoordinateCase(fixture, fixtureCase) {
  try {
    const output = executeCoordinateOperation(
      parseJsonBytesStrict(await readFixtureFile(fixture, fixtureCase.input)),
    );
    ...
    return caseResult(
      ...
      "accept",
      null,
      kind ? [{ kind, byte_length: bytes.length, sha256: sha256Hex(bytes) }] : [],
    );
  } catch (error) {
