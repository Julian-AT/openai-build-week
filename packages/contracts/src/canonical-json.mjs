...
function validateIJson(value) {
  ...
  while (stack.length > 0) {
    ...
    if (typeof current === "number" && !Number.isFinite(current))
      throw new RunnerFailure("numeric_out_of_range", "JCS input contains a non-finite number");
...
export function canonicalizeBytes(value) {
  ...
  if (typeof serialized !== "string")
    throw new RunnerFailure("json_parse", "RFC 8785 canonicalizer returned no JSON text");
...
export async function executeJcsCase(fixture, fixtureCase) {
  try {
    ...
    return caseResult(fixtureCase.case_id, "accept", null, [
      ...
      {
        kind: "digest",
        byte_length: digestBytes.length,
        sha256: sha256Hex(digestBytes),
        value_sha256: digest,
      },
    ]);
  } catch (error) {
