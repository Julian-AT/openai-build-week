...
export function parseJsonBytesStrict(bytes, { maxDepth = MAX_DOCUMENT_DEPTH } = {}) {
  ...
  const parseString = () => {
    ...
    while (index < source.length) {
      ...
      if (escape !== "u") throw syntaxFailure("invalid JSON escape");
  ...
  const parseValue = (depth) => {
    ...
    if (character === "{") {
      ...
      while (true) {
        ...
        if (keys.has(key)) throw new RunnerFailure("duplicate_name", "JSON object contains a duplicate member name");
        keys.add(key);
        whitespace();
    ...
    if (!Number.isFinite(value)) throw new RunnerFailure("numeric_out_of_range", "JSON number is not finite");
    return value;
  };
...
export function validateArchivePath(relativePath) {
  if (typeof relativePath !== "string" || Buffer.byteLength(relativePath, "utf8") > MAX_PATH_BYTES || !ARCHIVE_PATH.test(relativePath)) {
    throw new RunnerFailure("invalid_path", "unsafe archive-relative path");
  }
}

export async function resolveContained(base, relativePath, { archivePath = true, containmentRoot = base } = {}) {
  ...
  try {
    ...
    throw new RunnerFailure("invalid_path", "file reference cannot be resolved safely", { cause: error });
  }
...
export async function readFixtureFile(fixture, reference) {
  ...
  const bytes = await readBytesBounded(filePath, Math.min(MAX_FILE_BYTES, reference.byte_length + 1));
  if (bytes.length !== reference.byte_length || sha256Hex(bytes) !== reference.sha256) {
    throw new RunnerFailure("digest_mismatch", "fixture file does not match its immutable reference");
  }
  return bytes;
}

export async function readFixtureInternal(fixture, relativePath, base = fixture.fixtureRoot) {
  const filePath = await resolveContained(base, relativePath, { archivePath: false, containmentRoot: fixture.repoRoot });
...
export async function loadFixture(manifestPath, { repoRoot } = {}) {
  ...
  if (!validate(manifest)) throw new RunnerFailure("schema_validation", "fixture manifest does not satisfy FixtureManifestV1");
  if (manifest.cases.length > MAX_CASES) throw new RunnerFailure("schema_validation", "fixture contains too many cases");
  const caseIds = manifest.cases.map(({ case_id: caseId }) => caseId);
  if (new Set(caseIds).size !== caseIds.length || caseIds.some((caseId, index) => index > 0 && caseIds[index - 1] >= caseId)) {
    throw new RunnerFailure("semantic_invariant", "fixture case IDs must be unique and strictly lexicographic");
  }

  const schemas = new Map();
  for (const reference of manifest.schema_hashes) {
    const registered = REGISTERED_SCHEMAS.get(reference.contract_id);
    if (!registered || registered[0] !== reference.schema_id || registered[1] !== reference.relative_path) {
      throw new RunnerFailure("schema_validation", "fixture schema registry tuple is not canonical");
    }
    const schemaPath = await resolveContained(root, reference.relative_path);
    const schemaBytes = await readBytesBounded(schemaPath);
    if (sha256Hex(schemaBytes) !== reference.sha256) throw new RunnerFailure("digest_mismatch", "frozen schema digest mismatch");
    const schema = parseJsonBytesStrict(schemaBytes);
    if (schema.$id !== reference.schema_id) throw new RunnerFailure("schema_validation", "frozen schema ID mismatch");
...
function decodePointer(pointer) {
  if (pointer === "") return [];
  if (typeof pointer !== "string" || !pointer.startsWith("/")) throw new RunnerFailure("schema_validation", "invalid mutation pointer");
...
export function applyJsonMutations(source, mutations) {
  ...
  for (const mutation of mutations) {
    if (!["add", "replace"].includes(mutation.op)) throw new RunnerFailure("schema_validation", "unsupported JSON mutation operation");
    const parts = decodePointer(mutation.pointer);
    if (parts.length === 0) throw new RunnerFailure("schema_validation", "root mutation is not supported");
    let target = value;
    for (const part of parts.slice(0, -1)) {
      if (target === null || typeof target !== "object" || !(part in target)) throw new RunnerFailure("schema_validation", "mutation pointer does not resolve");
      target = target[part];
    }
    const key = parts.at(-1);
    if (mutation.op === "replace" && (target === null || typeof target !== "object" || !(key in target))) {
...
export function caseResult(caseId, verdict, rejectionClass = null, outputArtifacts = []) {
  return { case_id: caseId, verdict, rejection_class: rejectionClass, output_artifacts: outputArtifacts };
}
