...
test("GET /health reports readiness without authentication", async (t) => {
...
test("POST /v1/proposals rejects a missing bearer credential", async (t) => {
  let calls = 0;
...
test("POST /v1/proposals validates and forwards a closed trusted request", async (t) => {
...
test("POST /v1/proposals rejects duplicate JSON member names before validation", async (t) => {
  let calls = 0;
  const server = createGatewayServer({
    gatewayToken: "test-token",
    proposalService: {
...
test("POST /v1/proposals rejects non-UTF-8 JSON bytes", async (t) => {
  let calls = 0;
  const server = createGatewayServer({
    gatewayToken: "test-token",
    proposalService: {
...
test("POST /v1/proposals rejects client catalog allowlists and unknown fields", async (t) => {
  let calls = 0;
  const server = createGatewayServer({
    gatewayToken: "test-token",
    proposalService: {
...
test("known routes reject query-decorated URLs without invoking services", async (t) => {
  let calls = 0;
  const server = createGatewayServer({
    gatewayToken: "test-token",
    proposalService: {
...
test("POST /v1/proposals requires a JSON content type", async (t) => {
  const server = createGatewayServer({
    gatewayToken: "test-token",
...
test("POST /v1/proposals rejects a body over the fixed byte limit", async (t) => {
  const server = createGatewayServer({
    gatewayToken: "test-token",
    proposalService: { propose: async () => ({}) },
  });
...
test("POST /v1/proposals rejects a non-JPEG image data URL", async (t) => {
  let calls = 0;
  const server = createGatewayServer({
    gatewayToken: "test-token",
    proposalService: {
...
test("POST /v1/proposals binds vision ingress to exactly one JPEG", async (t) => {
  let calls = 0;
  const server = createGatewayServer({
    gatewayToken: "test-token",
    proposalService: {
...
test("POST /v1/proposals rejects an empty semantic prompt", async (t) => {
  const server = createGatewayServer({
    gatewayToken: "test-token",
...
test("POST /v1/realtime/client-secret returns only the validated ephemeral subset", async (t) => {
  const expected = {
    value: "ek_ephemeral",
...
test("known paths reject unsupported methods while unknown paths remain hidden", async (t) => {
  const server = createGatewayServer({ gatewayToken: "test-token" });
  const baseURL = await listen(server);
  t.after(() => server.close());
...
test("structured request logs exclude credentials, prompts, images, and ephemeral values", async (t) => {
  const logs: unknown[] = [];
...
test("an upstream deadline aborts work and returns a generic timeout", async (t) => {
...
test("upstream failures expose no provider details", async (t) => {
  const server = createGatewayServer({
    gatewayToken: "test-token",
    proposalService: {
      propose: async () => {
